    ...
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <Card title={editingId !== null ? 'Edit Expense' : 'Add Expense'} className="shadow-lg">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <Input label="Description" value={description} onChange={e => setDescription(e.target.value)} placeholder="What was this expense for?" required />
            <Input label="Amount (Rp)" type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0" required min="0" />
          </div>
          <div className="flex space-x-2">
            <Button type="submit" variant="primary">{editingId !== null ? 'Update Expense' : 'Add Expense'}</Button>
            {editingId !== null && (
              <Button type="button" variant="secondary" onClick={() => { setEditingId(null); setDescription(''); setAmount(''); }}>Cancel</Button>
            )}
          </div>
        </form>
      </Card>
      ...
      <Card title="Expense Records" className="shadow-lg overflow-hidden">
        <Table>
          ...
          {expenses.length === 0 ? (
            <TableRow>
              <TableCell colSpan={5} className="text-center text-gray-500 py-8">No expenses recorded.</TableCell>
            </TableRow>
          ) : (
            expenses.map(e => (
              <TableRow key={e.id}>
                ...
                <TableCell><Badge variant="danger">Rp {Number(e.amount).toLocaleString()}</Badge></TableCell>
