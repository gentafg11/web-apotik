...
export default function SalesPage() {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await axios.put(`/api/sales/${editingId}`, {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard title="Total Sales" value={sales.length} icon="🧾" />
      ...
      {printSale && (
        <Receipt sale={printSale} onClose={() => setPrintSale(null)} getProductName={getProductName} />
      )}
            ...
            sales.map(s => (
              <TableRow key={s.id}>
                ...
                <TableCell className="space-x-2">
                  <Button size="sm" variant="primary" onClick={() => setPrintSale(s)}>Print</Button>
