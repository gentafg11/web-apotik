...
export default function ProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = { name, sku, price: Number(price), cost: Number(cost), stock: Number(stock) };
    ...
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <Card title={editingId !== null ? 'Edit Product' : 'Add Product'} className="shadow-lg">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <Input label="Product Name" value={name} onChange={e => setName(e.target.value)} placeholder="Enter product name" required />
            <Input label="SKU" value={sku} onChange={e => setSku(e.target.value)} placeholder="e.g. PRD-001" required />
            <Input label="Price (Rp)" type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="0" required />
            <Input label="Cost (Rp)" type="number" value={cost} onChange={e => setCost(e.target.value)} placeholder="0" required />
            <Input label="Stock" type="number" value={stock} onChange={e => setStock(e.target.value)} placeholder="0" required />
            <div>
              ...
              {(imagePreview || existingImage) && (
                <img src={imagePreview || existingImage || ''} alt="Preview" className="h-20 w-20 object-cover rounded mt-2" />
              )}
            </div>
          </div>
          <div className="flex space-x-2">
            <Button type="submit" variant="primary">{editingId !== null ? 'Update Product' : 'Add Product'}</Button>
            {editingId !== null && (
              <Button type="button" variant="secondary" onClick={() => { setEditingId(null); setName(''); setSku(''); setPrice(''); setCost(''); setStock(''); setImageFile(null); setImagePreview(null); }}>Cancel</Button>
            )}
          </div>
        </form>
      </Card>
      ...
      <Card title="Product List" className="shadow-lg overflow-hidden">
        <Table>
          ...
          {products.length === 0 ? (
            <TableRow>
              <TableCell colSpan={8} className="text-center text-gray-500 py-8">No products found.</TableCell>
            </TableRow>
          ) : (
            products.map(p => (
              <TableRow key={p.id}>
                ...
                <TableCell className="w-20">
                  {p.imageUrl && <img src={p.imageUrl} alt={p.name} className="h-12 w-12 object-cover rounded" />}
                </TableCell>
                ...
                <TableCell>
                  <Badge variant={p.stock < 10 ? 'danger' : 'success'}>{p.stock < 10 ? 'Low Stock' : 'In Stock'}</Badge>
                </TableCell>
